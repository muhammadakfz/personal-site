...
date: "2025-11-02"
description: "Lecture notes on longitudinal waves covering sound propagation in gases and solids, elastic wave theory, dispersion in discrete lattices, and boundary reflection with worked examples."
...
$$ E_p = \tfrac{1}{2}Bs^2 = \tfrac{1}{2}B\!\left(\frac{\partial \eta}{\partial x}\right)^{\!2} = \tfrac{1}{2}Bk^2\eta_m^2\sin^2(kx-\omega t). $$
...
Note that the longitudinal bulk speed $c_L = \sqrt{(\lambda+2\mu)/\rho}$ always exceeds the thin-rod estimate $\sqrt{Y/\rho}$ for $0 < \sigma < 1/2$.
...
Substituting the trial solution $\eta_r = \eta_{\max}\,e^{i(\omega t - kra)}$ produces the dispersion relation

$$ \boxed{\omega^2 = \frac{4K}{m}\sin^2\!\left(\frac{ka}{2}\right)}. $$

In the long-wavelength limit ($ka \ll 1$) this recovers $\omega \approx (K/m)^{1/2}\,ka = c_0\,k$, reproducing the continuum result. At the Brillouin zone boundary ($k = \pi/a$), the group velocity $d\omega/dk$ vanishes — a standing wave forms.
...
giving $p_i = Z_1\dot{\eta}_i$, $\;p_r = -Z_1\dot{\eta}_r$, and $\;p_t = Z_2\dot{\eta}_t$.
...
Using $I = p_{\mathrm{rms}}^2/Z = Z\langle\dot{\eta}^2\rangle_{\mathrm{rms}}$, the intensity reflection and transmission fractions are
